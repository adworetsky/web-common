...
def people_page(request):
    ...
    other = members.exclude(alumni=True).difference(
        pi, staff, postdoc, gradstudent, ra, undergrad
    )
...
def publications_page(request):
    ...
    return render(
        request,
        "common/publications.html",
        {"publications": publications, "years": years},
    )
...
def news_page(request):
    ...
    if year == "year" and month == "month":  # get last 5 news items if main
        ...
        news = news_item.objects.filter(pub_date__month=int(month)).order_by(
            "-pub_date"
        )
        month = int(month)
    elif month == "month":
...
def data_page(request):
    ...
    return render(
        request, "common/data_software.html", {"data": data, "software": software}
    )
